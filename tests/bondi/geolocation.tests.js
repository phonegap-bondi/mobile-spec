Tests.prototype.GeolocationTests = function() {
	...
	var reception = false;
	if (window.confirm("If you want to run the tests needing reception please click confirm, otherwise tests that need deactivated reception are run")){
		reception = true;
	}
	...
	test("Should be able to handle parameters correctly", function() {
		 if (reception){
			 ...
			 var exception = false;
			 try {
				 bondi.geolocation.getCurrentPosition(null, 
						 function (e){
					 ok( (typeof e != undefined), "Should deliver INVALID_ARGUMENT_ERROR when successCallback==null");
				 });
			 } catch(error) {
				 exception = true;
			 }
			 ok( !exception, "Should not throw INVALID_ARGUMENT_ERROR when successCallback==null but deliver it to errorCallback");

			 exception = false;
			 try {
				 bondi.geolocation.getCurrentPosition(function (p){}, 
						 function (e){
					 ok( (typeof e != undefined), "Should deliver INVALID_ARGUMENT_ERROR when timeout==-2");
				 },
				 {timeout:-2});
			 } catch(error) {
				 exception = true;
			 }
			 ok( !exception, "Should put error into errorcallback INVALID_ARGUMENT_ERROR when timeout==-2");

			 exception = false;
			 try {
				 bondi.geolocation.getCurrentPosition(function (p){}, 
						 function (e){
					 ok( (typeof e != "undefined"), "Should throw INVALID_ARGUMENT_ERROR when maximumAge==-2");
				 },
				 {maximumAge:-2});
			 } catch(error) {
				 exception = true;
			 }
			 ok( !exception, "Should put error into errorCallback... INVALID_ARGUMENT_ERROR when maximumAge==-2");
			 ...
			 exception = false;
			 try {
				 bondi.geolocation.getCurrentPosition(function (p){}, function (e){
					 ok( (typeof e != 'undefined') && e != null, "Should deliver INVALID_ARGUMENT_ERROR when PositionOptions='geotest'");
				 },'geotest');
			 } catch(error) {
				 exception = true;
			 }
			 ok( !exception, "Should not throw INVALID_ARGUMENT_ERROR when PositionOptions='geotest', but deliver it to errorCallback");
	...
	test("Should be able to handle parameters correctly", function() {
		if (reception){
			...
			var exception = false;
			try {
				bondi.geolocation.watchPosition(null, function (e){
					ok( (typeof e != undefined), "Should deliver INVALID_ARGUMENT_ERROR when successCallback==null");
				});
			} catch(error) {
				exception = true;		
			...
			exception = false;
			try {
				bondi.geolocation.watchPosition(function (p){}, function (e){ok((typeof e != "undefined"), "Should throw INVALID_ARGUMENT_ERROR when timeout=-2");},{timeout:-2});
			} catch(error) {
				exception = true;
			}
			ok( !exception, "Should deliver INVALID_ARGUMENT_ERROR to errorCallback when timeout=-2");

			exception = false;
			try {
				bondi.geolocation.watchPosition(function (p){}, function (e){ok( (typeof e != "undefined"), "Should deliver INVALID_ARGUMENT_ERROR when maximumAge=-2");},{maximumAge:-2});
			} catch(error) {
				exception = true;
			}
			ok( !exception, "Should not throw INVALID_ARGUMENT_ERROR but deliver it to the errorCallback when maximumAge=-2");

			exception = false;
			try {
				bondi.geolocation.watchPosition(function (p){}, null);
			} catch(error) {
				exception = true;
			}
			ok( !exception, "Should throw no exception when errorCallback=null");
			...
			exception = false;
			try {
				bondi.geolocation.watchPosition(function (p){}, function (e){
					ok( (typeof e != 'undefined') && e != null, "Should deliver INVALID_ARGUMENT_ERROR when PositionOptions='geotest'");
				},'geotest');
			} catch(error) {
				exception = true;
			}
			ok( !exception, "Should throw INVALID_ARGUMENT_ERROR when PositionOptions='geotest', but deliver the error to the errorcallback");
